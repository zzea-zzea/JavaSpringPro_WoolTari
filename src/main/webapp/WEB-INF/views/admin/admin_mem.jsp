										...
										<c:if test="${not empty MbList}">
											<c:forEach var="mb" items="${MbList}" varStatus="vs">
												<tr data-toggle="collapse" data-target="#collapseOne"
													...
													<td><c:if test="${mb.gender eq 2}">여자</c:if> <c:if
															test="${mb.gender eq 1}">남자</c:if></td>
													<td><c:out value="${mb.email}" /></td>
													<td><c:if test="${mb.isMember eq 1}">개인</c:if> <c:if
															test="${mb.isMember eq 2}">법인</c:if> <c:if
															test="${mb.isMember eq 8}">탈퇴회원</c:if> <c:if
															test="${mb.isMember eq 9}">관리자</c:if></td>
					...
					<div class="form_div">
						...
						<table class="table">
							<tbody>
								...
								<tr>
									<td>전화번호</td>
									<td class="phone_si"><input type="text" placeholder="010">&nbsp;-&nbsp;<input
										type="text" placeholder="0000">&nbsp;-&nbsp;<input
										type="text" placeholder="0000"></td>
								</tr>
								<tr>
									<td>성별</td>
									<td>여</td>
